    ...
    <PageLayout currentPage="Alumni Upload">
      ...
      <div style={{ backgroundColor: theme.white, borderRadius: 16, border: `1px solid ${theme.cardBorder}`, padding: 24, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: theme.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>1️⃣</div>
          <div style={{ flex: 1 }}>
        ...
        <div style={{ marginTop: 16, padding: 16, backgroundColor: theme.gray50, borderRadius: 10 }}>
          <p style={{ fontSize: 12, fontWeight: 600, color: theme.gray500, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Columns</p>
      ...
      <div style={{ backgroundColor: theme.white, borderRadius: 16, border: `1px solid ${theme.cardBorder}`, padding: 24, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: theme.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>2️⃣</div>
          <div>
        ...
        <div
          ...
          style={{ border: `2px dashed ${theme.gray300}`, borderRadius: 12, padding: 40, textAlign: 'center', cursor: 'pointer', backgroundColor: theme.gray50, transition: 'border-color 0.15s' }}
        ...
        >
          ...
          <p style={{ fontSize: 13, color: theme.gray400, marginTop: 6 }}>.xlsx files only — max 500 alumni per upload</p>
      ...
      {(preview.length > 0 || errors.length > 0) && (
        <div style={{ backgroundColor: theme.white, borderRadius: 16, border: `1px solid ${theme.cardBorder}`, padding: 24, marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: theme.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>3️⃣</div>
              <div>
                <h2 style={{ fontSize: 16, fontWeight: 600, color: theme.gray900, margin: 0 }}>Review & confirm</h2>
                <p style={{ fontSize: 13, color: theme.gray500, marginTop: 4 }}>{preview.length} valid · {errors.length} with errors</p>
              </div>
            </div>
          ...
          {errors.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 12, fontWeight: 600, color: theme.danger, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 8 }}>Rows with errors</p>
              {errors.map((e) => (
                ...
                <tbody>
                  {preview.slice(0, 20).map((a, i) => (
                    <tr key={i} style={{ borderBottom: `1px solid ${theme.gray100}`, backgroundColor: i % 2 === 0 ? theme.white : theme.gray50 }}>
                      ...
                      <td style={{ padding: '7px 12px', color: theme.gray600 }}>{a.graduationYear}</td>
                      <td style={{ padding: '7px 12px', color: theme.gray600, textTransform: 'capitalize' }}>{a.graduationSemester}</td>
                      <td style={{ padding: '7px 12px' }}><Badge label={a.position} variant="gold" /></td>
                      <td style={{ padding: '7px 12px', color: theme.gray600 }}>{a.currentEmployer ?? '—'}</td>
                      <td style={{ padding: '7px 12px', color: theme.gray600 }}>{a.currentCity && a.currentState ? `${a.currentCity}, ${a.currentState}` : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {preview.length > 20 && (
                <p style={{ fontSize: 13, color: theme.gray400, textAlign: 'center', padding: 12 }}>Showing first 20 of {preview.length}</p>
              )}
            </div>
          )}
        </div>
